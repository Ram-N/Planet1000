...
export function generateMetadata(): Metadata {
  const gameName = process.env.NEXT_PUBLIC_GAME_NAME ?? 'Planet1000';
  return {
    title: `${gameName} — See the World Differently`,
    description:
...
export default function RootLayout({ children }: LayoutProps<"/">) {
  const siteTitle = process.env.NEXT_PUBLIC_GAME_NAME ?? 'Planet1000';

  return (
    <html lang="en" className={`${geist.variable} h-full antialiased`}>
      <body className="min-h-full flex flex-col bg-slate-50 font-sans" data-gramm="false">
